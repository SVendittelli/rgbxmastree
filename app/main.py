...
async def lifespan(app: FastAPI):
    ...
    cursor = conn.cursor()
    cursor.execute("CREATE TABLE IF NOT EXISTS current_pattern(id INTEGER PRIMARY KEY, pattern TEXT NOT NULL)")
    conn.commit()

    previous_pattern, = cursor.execute("SELECT pattern FROM current_pattern").fetchone()
...
async def start_pattern(pattern: str):
    ...
    conn = app.state.conn
    cursor = conn.cursor()
    cursor.execute("INSERT INTO current_pattern VALUES (1, :pattern) ON CONFLICT DO UPDATE SET pattern = :pattern", { 'pattern': pattern })
    conn.commit()
